...
export interface UIState {
  ...
  // 我的年級（用於修課資格提醒）：0=未設定, 1~4=大一~大四, 10=研究所
  myGrade: number;
  // 時間段篩選相關
...
const loadMyGrade = (): number => {
  try {
    return parseInt(localStorage.getItem('TTUCourseSelector.myGrade') || '0', 10) || 0;
  } catch {
    return 0;
  }
};
...
const uiSlice = createSlice({
  ...
  reducers: {
    ...
    setMyGrade: (state, action: PayloadAction<number>) => {
      state.myGrade = action.payload;
      try {
        localStorage.setItem('TTUCourseSelector.myGrade', String(action.payload));
      } catch {
        // ignore
      }
    },
